...
1. **建图（Ingest）** — 从四类来源抽取业务实体、关系与原文片段，合并进按命名空间隔离的知识图谱：
   - **代码仓库**：语言无关层（目录 / 文件 / 包 / import 关系）+ Go AST 层（函数 / 类型 / 调用关系）。
   - **历史对话**：Claude Code、Codex、WorkBuddy、CodeBuddy 的会话记录，蒸馏出设计决策与结论。
   - **Obsidian 笔记**：把 vault 里的知识并入图谱。
   - **MCP 实时写入**：agent 在对话中学到新知识后直接调 `remember_knowledge` 回写。

2. **检索（Recall）** — **HybridRAG 双通道**：实体结构通道（语义 seed + **正向有向**关系扩展）与原文 chunk 通道并行召回，用 **RRF（Reciprocal Rank Fusion）** 融合排序。关系扩展尊重方向性——命中 A 时只沿 `A→B` 方向拉入 B，不会反向把所有指向 A 的父级都带出来。embedding 默认走云端（OpenAI-compatible），无 key 时本地词面兜底。

3. **注入（Inject）** — 通过 stdio MCP server 把知识图谱暴露给所有 AI Agent，AI 在会话中直接调用工具查询，带**来源溯源**。默认搜索所有命名空间，确保信息完整不遗漏。
